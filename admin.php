...
if ($action === 'add_user' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($adm) $is_guest = 0; // admins can't be guests
    if ($uname && strlen($pass) >= 4) {
        try {
            $stmt = $pdo->prepare('INSERT INTO users (username, password_hash, is_admin, is_guest) VALUES (?,?,?,?)');
            $stmt->execute([$uname, password_hash($pass, PASSWORD_BCRYPT), $adm, $is_guest]);
...
if ($action === 'toggle_admin' && isset($_POST['user_id'])) {
    $uid = (int)$_POST['user_id'];
    $pdo->prepare('UPDATE users SET is_admin = 1 - is_admin, is_guest = 0 WHERE id = ?')->execute([$uid]);
    flash('success', 'Admin status updated.');
    redirect(BASE_URL . '/admin.php');
}

if ($action === 'toggle_guest' && isset($_POST['user_id'])) {
    $uid = (int)$_POST['user_id'];
    $pdo->prepare('UPDATE users SET is_guest = 1 - is_guest, is_admin = 0 WHERE id = ?')->execute([$uid]);
...
if ($action === 'edit_user' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $pass  = $_POST['password'] ?? '';
    $adm   = isset($_POST['is_admin']) ? 1 : 0;
    $guest = isset($_POST['is_guest']) ? 1 : 0;
    if ($adm) $guest = 0;
    if ($uid && $uname) {
        try {
            if ($pass !== '') {
                $pdo->prepare('UPDATE users SET username=?, password_hash=?, is_admin=?, is_guest=? WHERE id=?')
                    ->execute([$uname, password_hash($pass, PASSWORD_BCRYPT), $adm, $guest, $uid]);
            } else {
                $pdo->prepare('UPDATE users SET username=?, is_admin=?, is_guest=? WHERE id=?')
                    ->execute([$uname, $adm, $guest, $uid]);
            }
            flash('success', "User '$uname' updated.");
        } catch (PDOException $e) {
            flash('error', 'Username already exists.');
        }
    }
...
// For editing: load a single user
$edit_user = null;
    ...
    <div class="table-wrap">
      <table class="data-table">
        ...
        <tr>
          ...
          <td>
            <?php if ($u['is_admin']): ?>
              <span class="bool-yes">Admin</span>
            <?php elseif ($u['is_guest']): ?>
              <span style="color:#7a5c2e;font-weight:600">Guest</span>
            <?php else: ?>
              <span class="bool-no">User</span>
            <?php endif; ?>
          </td>
          <td class="actions">
            <?php if ($u['id'] != $_SESSION['user_id']): ?>
            <a href="?edit_user=<?= $u['id'] ?>" class="btn btn-ghost btn-sm">Edit</a>
...
<!-- ── Row 2: New/Edit User + New/Edit Collection ── -->
<div class="admin-grid mt-16">

  <!-- New / Edit User -->
  <div class="card">
    <?php if ($edit_user): ?>
      <h2 style="font-family:var(--font-head);margin-bottom:16px">Edit User</h2>
      <form method="post">
        <input type="hidden" name="action"  value="edit_user">
        <input type="hidden" name="user_id" value="<?= $edit_user['id'] ?>">
    <?php else: ?>
      <h2 style="font-family:var(--font-head);margin-bottom:16px">New User</h2>
      <form method="post">
    ...
    <?php endif; ?>
      ...
      <div class="form-group">
        <label>Password<?= $edit_user ? ' <span class="text-muted">(leave empty to keep current)</span>' : '' ?></label>
        <input type="password" name="password" class="form-control"
               <?= $edit_user ? '' : 'required' ?> minlength="4">
      </div>
      <div class="form-group" style="display:flex;align-items:center;gap:8px">
        <input type="checkbox" name="is_admin" id="chk_admin" <?= !empty($edit_user['is_admin']) ? 'checked' : '' ?>>
        <label for="chk_admin" style="margin:0;text-transform:none;font-size:.95rem;font-weight:400">Admin</label>
      </div>
      <div class="form-group" style="display:flex;align-items:center;gap:8px">
        <input type="checkbox" name="is_guest" id="chk_guest" <?= !empty($edit_user['is_guest']) ? 'checked' : '' ?>>
        <label for="chk_guest" style="margin:0;text-transform:none;font-size:.95rem;font-weight:400">Guest (read-only)</label>
      </div>
      <div style="display:flex;gap:10px;margin-top:8px">
        <button class="btn btn-primary"><?= $edit_user ? 'Save Changes' : 'Create User' ?></button>
        <?php if ($edit_user): ?>
  ...
  <div class="card">
    <?php if ($edit_coll): ?>
      <h2 style="font-family:var(--font-head);margin-bottom:16px">Edit Collection</h2>
      <form method="post">
        <input type="hidden" name="action"  value="edit_collection">
        <input type="hidden" name="coll_id" value="<?= $edit_coll['id'] ?>">
    <?php else: ?>
      <h2 style="font-family:var(--font-head);margin-bottom:16px">New Collection</h2>
      <form method="post">
    ...
    <?php endif; ?>
      ...
      <div class="form-group" style="display:flex;align-items:center;gap:8px">
        ...
        <label for="chk_images" style="margin:0;text-transform:none;font-size:.95rem;font-weight:400">Items have images</label>
      </div>
...
<!-- ── Row 3: Global Tags + New Global Tag ── -->
<div class="admin-grid mt-16" id="global-tags">

  <!-- Global tags list -->
  <div class="card">
    <h2 style="font-family:var(--font-head);margin-bottom:16px">Global Tags
      <span class="text-muted" style="font-size:.85rem;font-weight:400"> — visible across all collections</span>
    </h2>
  ...
  <!-- New Global Tag -->
  <div class="card">
    <h2 style="font-family:var(--font-head);margin-bottom:16px">New Global Tag</h2>
...
#fields-list li { flex-wrap: wrap; row-gap: 6px; }
.field-count-badge {
  display: inline-flex; align-items: center; justify-content: center;
  background: #e8a87c; color: #5a2800; font-size: .75rem; font-weight: 700;
  min-width: 22px; height: 22px; border-radius: 11px; padding: 0 6px; cursor: default;
}
.field-warning {
  width: 100%; background: #fdf3e7; border: 1px solid #e8a87c;
  border-radius: 4px; padding: 7px 10px; font-size: .83rem; color: #7a3800; margin-top: 2px;
}
...
function warnTypeChange(select, valueCount, fieldName) {
  ...
  if (newType === origType) { warnDiv.style.display = 'none'; return; }
  let msg = '';
  if (origType === 'boolean' && newType !== 'boolean') {
    ...
    msg = `<strong>Note:</strong> Changing "${fieldName}" to <em>Boolean</em> — existing values will be treated as true/false.`;
  } else if (origType === 'text' && newType === 'number') {
    msg = `<strong>Note:</strong> "${fieldName}" has ${valueCount} item${valueCount!==1?'s':''} with text values. Non-numeric values will show as empty in forms.`;
  } else {
    msg = `<strong>Note:</strong> "${fieldName}" has ${valueCount} item${valueCount!==1?'s':''} with existing values. Data is preserved but may display differently as <em>${newType}</em>.`;
  }
